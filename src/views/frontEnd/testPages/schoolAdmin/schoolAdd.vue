<template>
  <div>
    ...
    <fieldset class="title">
      <legend v-if="ruleForm.roleType === 1">添加管理员</legend>
      <legend v-else>添加管理员</legend>
    </fieldset>
    <!-- 表单 -->
    <el-form :model="ruleForm" ref="ruleForm" :rules="rules" label-width="100px">
      <el-form-item label="账号" prop="username">
        <el-input v-model="ruleForm.username" class="input" placeholder="管理员账号"></el-input>
        <div class="must">必填</div>
      </el-form-item>
      <el-form-item label="密码" prop="password">
        <el-input v-model="ruleForm.password" type="password" class="input"
          placeholder="密码需包含数字、字母、符号，且长度为6 - 20位"></el-input>
        <div class="must">必填</div>
      </el-form-item>
      ...
      this.$refs[formName].validate(async (valid, invalidFields) => {
        if (valid) {
          ...
          try {
            ...
            // 处理可能发生的错误
            console.error('请求数据失败:', msg || error);
            // 错误通知
